...
TEST(lstm, train) {
  ...
  nn << recurrent_layer(lstm(3, 2), 1, params);
...
TEST(lstm, train_different_batches) {
...
TEST(lstm, train2) {
  ...
  nn << recurrent_layer(lstm(4, 6), 1) << recurrent_layer(lstm(6, 3), 1);
  nn.weight_init(weight_init::xavier());
...
TEST(lstm, gradient_check) {
  network<sequential> nn;
  nn << recurrent_layer(lstm(50, 10), 1) << tanh_layer();
