...
class GRUCellOp : public core::OpKernel {
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    tensor_t &h      = context.output(1);  // output state  h(t)
    tensor_t &r      = context.output(2);  // reset gate    r(t)
    tensor_t &z      = context.output(3);  // update gate   z(t)
    tensor_t &pre_h  = context.output(4);  // aux state  pre_h(t)
    tensor_t &hr     = context.output(5);  // aux state  pre_h(t)
    tensor_t &pre_r  = context.output(6);  // aux state  pre_r(t)
    tensor_t &pre_z  = context.output(7);  // aux state  pre_z(t)
    tensor_t &post_z = context.output(8);  // aux state  - post_z(t) (1-z)
    ...
    fill_tensor(pre_h, float_t{0});   // aux state  pre_h(t)
    fill_tensor(hr, float_t{0});      // aux state  hr(t)
    fill_tensor(pre_r, float_t{0});   // aux state  pre_r(t)
    fill_tensor(pre_z, float_t{0});   // aux state  pre_z(t)
