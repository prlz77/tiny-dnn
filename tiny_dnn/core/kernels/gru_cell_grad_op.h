...
class GRUCellGradOp : public core::OpKernel {
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    const tensor_t &h      = context.output(1);  // output state  h(t)
    const tensor_t &r      = context.output(2);  // reset gate    r(t)
    const tensor_t &z      = context.output(3);  // update gate   z(t)
    const tensor_t &pre_h  = context.output(4);  // aux state  pre_h(t)
    const tensor_t &hr     = context.output(5);  // aux state  pre_h(t)
    const tensor_t &pre_r  = context.output(6);  // aux state  pre_r(t)
    const tensor_t &pre_z  = context.output(7);  // aux state  pre_z(t)
    const tensor_t &post_z = context.output(8);  // aux state  - post_z(t) (1-z)
    ...
    kernels::gru_cell_op_internal(
      ...
      params.has_bias_ ? *db_2h : dummy, d_o_next, d_x_prev, d_h_prev, h, r, z,
      pre_h, hr, pre_r, pre_z, post_z, params, context.parallelize());
  }
};
