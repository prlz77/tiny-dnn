...
class LSTMCellOp : public core::OpKernel {
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    tensor_t &pre_i    = context.output(3);
    tensor_t &pre_f    = context.output(4);
    tensor_t &pre_z    = context.output(5);
    tensor_t &pre_o    = context.output(6);
    tensor_t &post_i   = context.output(7);
    tensor_t &post_f   = context.output(8);
    tensor_t &post_z   = context.output(9);
    tensor_t &post_c   = context.output(10);

    // initialize outputs
    fill_tensor(out_data, float_t{0});
    fill_tensor(h_next, float_t{0});
    fill_tensor(c_next, float_t{0});
    fill_tensor(pre_i, float_t{0});
    fill_tensor(pre_f, float_t{0});
    fill_tensor(pre_z, float_t{0});
    fill_tensor(pre_o, float_t{0});
    fill_tensor(post_i, float_t{0});
    fill_tensor(post_f, float_t{0});
    fill_tensor(post_z, float_t{0});
    fill_tensor(post_c, float_t{0});
    ...
    if (engine == core::backend_t::internal || engine == core::backend_t::avx) {
      kernels::lstm_cell_op_internal(
        ...
        params.has_bias_ ? (*b_2o)[0] : vec_t(), out_data, h_next, c_next,
        pre_i, pre_f, pre_z, pre_o, post_i, post_f, post_z, post_c, params,
        context.parallelize());
    } else {
