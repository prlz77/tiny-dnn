...
class LSTMCellGradOp : public core::OpKernel {
 public:
  ...
  void compute(core::OpKernelContext &context) override {
    ...
    const tensor_t &pre_z  = context.output(5);
    const tensor_t &pre_o  = context.output(6);  // post_o is o_next
    const tensor_t &post_i = context.output(7);
    const tensor_t &post_f = context.output(9);
    const tensor_t &post_z = context.output(8);
    const tensor_t &post_c = context.output(10);  // pre_c is c_next
