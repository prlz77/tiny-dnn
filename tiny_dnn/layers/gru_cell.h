...
class gru_cell : public cell {
 public:
   ...
   * @param params [in] optional gru cell parameters
   **/
  ...
  inline std::vector<vector_type> output_order() {
    return {vector_type::data,  // output vector s(t)
            ...
            vector_type::aux,   // aux state  pre_h(t)
            vector_type::aux,   // aux state  hr(t)
            vector_type::aux,   // aux state  pre_r(t)
            vector_type::aux,   // aux state  pre_z(t)
            vector_type::aux};  // aux state  - post_z(t) (1-z)
  }
  ...
  inline std::vector<index3d<size_t>> out_shape() const {
    return {index3d<size_t>(params_.out_size_, 1, 1),  // output vector s(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // output state  h(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // reset gate    r(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // update gate   z(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // aux state  pre_h(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // aux state  hr(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // aux state  pre_r(t)
            index3d<size_t>(params_.out_size_, 1, 1),  // aux state  pre_z(t)
            index3d<size_t>(params_.out_size_, 1,
                            1)};  // aux state  - post_z(t) (1-z)
  }
